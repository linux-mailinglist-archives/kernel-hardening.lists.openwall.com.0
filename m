Return-Path: <kernel-hardening-return-19604-lists+kernel-hardening=lfdr.de@lists.openwall.com>
X-Original-To: lists+kernel-hardening@lfdr.de
Delivered-To: lists+kernel-hardening@lfdr.de
Received: from mother.openwall.net (mother.openwall.net [195.42.179.200])
	by mail.lfdr.de (Postfix) with SMTP id 669EF2420BC
	for <lists+kernel-hardening@lfdr.de>; Tue, 11 Aug 2020 22:02:25 +0200 (CEST)
Received: (qmail 28120 invoked by uid 550); 11 Aug 2020 20:02:19 -0000
...
Received: (qmail 28096 invoked from network); 11 Aug 2020 20:02:18 -0000
...
Cc: linux-kernel@vger.kernel.org,  Aleksa Sarai <cyphar@cyphar.com>,  Alexei
 ...
 <vincent.strubel@ssi.gouv.fr>,  kernel-hardening@lists.openwall.com,
  ...
  Thibaut Sautereau <thibaut.sautereau@ssi.gouv.fr>,  Randy Dunlap
...
> https://lore.kernel.org/lkml/1477d3d7-4b36-afad-7077-a38f42322238@digikod=
.net/
>
...
> @@ -165,6 +166,54 @@ system needs to prune the inode list instead of allo=
cating
>  more.
>=20=20
>=20=20
...
> +``O_MAYEXEC`` flag can be passed to :manpage:`openat2(2)` to only open r=
egular
> +files that are expected to be executable.  If the file is not identified=
 as
> +executable, then the syscall returns -EACCES.  This may allow a script
> +interpreter to check executable permission before reading commands from =
a file,
> +or a dynamic linker to only load executable shared objects.  One interes=
ting
...
policy,
> +which first starts by restricting mount points with the ``noexec`` optio=
n.
...
kernel
> +or mapped as executable memory (e.g. libraries).  With script interprete=
rs
> +using the ``O_MAYEXEC`` flag, the executable permission can then be chec=
ked
> +before reading commands from files. This makes it possible to enforce the
> +``noexec`` at the interpreter level, and thus propagates this security p=
olicy
> +to scripts.  To be fully effective, these interpreters also need to hand=
le the
> +other ways to execute code: command line parameters (e.g., option ``-e``=
 for
> +Perl), module loading (e.g., option ``-m`` for Python), stdin, file sour=
cing,
> +environment variables, configuration files, etc.  According to the threat
> +model, it may be acceptable to allow some script interpreters (e.g. Bash=
) to
> +interpret commands from stdin, may it be a TTY or a pipe, because it may=
 not be
> +enough to (directly) perform syscalls.
> +
> +There are two complementary security policies: enforce the ``noexec`` mo=
unt
> +option, and enforce executable file permission.  These policies are hand=
led by
> +the ``fs.open_mayexec_enforce`` sysctl (writable only with ``CAP_SYS_ADM=
IN``)
> +as a bitmask:
> +
> +1 - Mount restriction: checks that the mount options for the underlying =
VFS
> +    mount do not prevent execution.
> +
> +2 - File permission restriction: checks that the to-be-opened file is ma=
rked as
> +    executable for the current process (e.g., POSIX permissions).
> +
> +Note that as long as a policy is enforced, opening any non-regular file =
with
> +``O_MAYEXEC`` is denied (e.g. TTYs, pipe), even when such a file is mark=
ed as
> +executable or is on an executable mount point.
> +
> +Code samples can be found in tools/testing/selftests/openat2/omayexec_te=
st.c
> +and interpreter patches (for the original O_MAYEXEC version) may be foun=
d at
> +https://github.com/clipos-archive/clipos4_portage-overlay/search?q=3DO_M=
AYEXEC .
...
>  #include "internal.h"
>  #include "mount.h"
> @@ -425,6 +426,11 @@ static int sb_permission(struct super_block *sb, str=
uct inode *inode, int mask)
>  	return 0;
>  }
>=20=20
...
> --- a/kernel/sysctl.c
> +++ b/kernel/sysctl.c
> @@ -113,6 +113,7 @@ static int sixty =3D 60;
>=20=20
...
> @@ -888,7 +889,6 @@ static int proc_taint(struct ctl_table *table, int wr=
ite,
>  	return err;
>  }
>=20=20
> -#ifdef CONFIG_PRINTK
>  static int proc_dointvec_minmax_sysadmin(struct ctl_table *table, int wr=
ite,
>  				void *buffer, size_t *lenp, loff_t *ppos)
>  {
> @@ -897,7 +897,6 @@ static int proc_dointvec_minmax_sysadmin(struct ctl_t=
...
>=20=20
>  /**
>   * struct do_proc_dointvec_minmax_conv_param - proc_dointvec_minmax() ra=
nge checking structure
> @@ -3264,6 +3263,15 @@ static struct ctl_table fs_table[] =3D {
>  		.extra1		=3D SYSCTL_ZERO,
>  		.extra2		=3D &two,
>  	},
> +	{
> +		.procname       =3D "open_mayexec_enforce",
> +		.data           =3D &sysctl_open_mayexec_enforce,
